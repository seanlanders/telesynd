...
#	~~~
#	Modified by S. Landers (sean.landers@gmail.com) 09-09-2020
...
#	parser.add_argument ("-d", "--double", help="Defines data in 'words' rather than bytes", action="store_true")
	...
	parser.add_argument ("-t", "--tablename", help="The tablename to include in the hex", type=str, default="table")
	...
	for f in args.infile:
		if f == '-':
			sys.exit()
		bmp2hex(f.name, tablewidth, sizebytes, invert, raw, named, double, xbm, tablename)
...
def bmp2hex(infile, tablewidth, sizebytes, invert, raw, named, double, xbm, tablename):
	...
	if (raw):
		...
		output = '#include "' + tablename + '.h"\n\n'
		output += 'PROGMEM unsigned char const ' + tablename + '[] = {'

		# Output the size of the BMP
		if (not (sizebytes%2)):
			output += "{0:#04X}".format((pixelWidth>>8) & 0xFF) + ", " + "{0:#04X}".format(pixelWidth & 0xFF) + ", " + \
		    	  "{0:#04X}".format((pixelHeight>>8) & 0xFF) + ", " + "{0:#04X}".format(pixelHeight & 0xFF) + ","
		else:
			output += "{0:#04X}".format(pixelWidth & 0xFF) + ", " + "{0:#04X}".format(pixelHeight & 0xFF) + ","
	...
	# Wrap the output buffer. Print. Then, finish.
		...
		if (named):
			output += '\n};'
			output += DEFAULTS.STRUCTURE_NAME + ' const ' + tablename + ' = {{{0}, {1}, {2}, 0, '.format(pixelWidth, pixelHeight, bitDepth) + \
